...
/*! BETAMODE JS SWAP*/
if (void 0 === fversion) {
    var fversion = new Array;
    if (document.location.href.indexOf("betamode=") > -1) {
        var betavers = encodeURIComponent(document.location.href.split("betamode=")[1].split("&")[0]);
        if (document.location.href.indexOf("betamode=webstandards") > -1 && document.location.href.indexOf("https") > -1) var betapath = "https://webstandards.us.oracle.com:89/assets";
        else if (document.location.href.indexOf("betamode=webstandards") > -1) var betapath = "http://webstandards.us.oracle.com:8989/assets";
        else if (document.location.href.indexOf("betamode=beta") > -1) {
            betavers = betavers.indexOf(".") > -1 ? betavers.split(".")[1] : betavers;
            var betapath = "https://www.oracle.com/asset/web/beta" + betavers
        } else if (document.location.href.indexOf("betamode=prod") > -1) var betapath = "https://www.oracle.com/asset/web";
        else var betapath = "//www.oracle.com/webfolder/s/assets/" + betavers
    }
}

function checkbetamode(a, b) {
    if (document.location.href.indexOf("betamode=") > 0 && !fversion[a] || document.location.href.indexOf("betamode=") > 0 && "betamode" != fversion[a]) {
        fversion[a] = "betamode";
        document.write('<script data-bmode="true" src="' + betapath + "/js/" + a + '.js"><\/script>');
        throw new Error("BETA MODE - DEFAULT " + a.toUpperCase() + " SCRIPT CANCELLED ")
    }
    fversion[a] = b
}
checkbetamode("jquery", "20.3.0");
...
/*! jQuery Migrate - Mute version log */
! function (a) {
    var b = a.console.log;
    console.log = function log() {
        if ("string" == typeof arguments[0] && arguments[0].match(/Migrate is installed/)) return !1;
...
/*! BETAMODE CSS SWAP */
if (document.location.href.indexOf("betamode=") > -1) {
    jQuery('head link[href*="css"],link[href*="legacy-ocom-mosaic"],link[href*="legacy-master-mosaic"]').each(function () {
        var a = jQuery(this).attr("href").replace(/.*\/([^\/]+)$/g, "$1");
        if (a.indexOf("safari-fixes") < 0 && a.indexOf("otn-hp-otn-hp-css") < 0) {
            jQuery(this).remove();
            document.write('<link href="' + betapath + "/css/" + a + '" type="text/css" rel="stylesheet" />')
        }
    });
    jQuery(document).ready(function ($) {
        $(document).on("click", "a[href]", function () {
            if ("" != $(this).attr("href") && $(this).attr("href").indexOf("javascript") < 0 && $(this).attr("href").indexOf("iframe") < 0 && $(this).attr("href").indexOf("betamode") < 0) {
                var a = $(this).attr("href") && $(this).attr("href").indexOf("?") > -1 ? "&" : "?";
                $(this).attr("href") && $(this).attr("href").indexOf("#") > -1 ? $(this).attr("href", $(this).attr("href").split("#")[0] + a + "betamode=" + betavers + "#" + $(this).attr("href").split("#")[1]) : $(this).attr("href") && $(this).attr("href", $(this).attr("href") + a + "betamode=" + betavers)
            }
        });
        $("a[onclick][href]").each(function () {
            if ($(this).attr("href").indexOf("javascript") < 0 && $(this).attr("href").indexOf("iframe") < 0 && $(this).attr("href").indexOf("betamode") < 0) {
                var a = $(this).attr("href") && $(this).attr("href").indexOf("?") > -1 ? "&" : "?";
                $(this).attr("href") && $(this).attr("href").indexOf("#") > -1 ? $(this).attr("href", $(this).attr("href").split("#")[0] + a + "betamode=" + betavers + "#" + $(this).attr("href").split("#")[1]) : $(this).attr("href") && $(this).attr("href", $(this).attr("href") + a + "betamode=" + betavers)
            }
        });
        $("body").append('<div id="betamode">BETA MODE - ' + betavers + "<span></div>").addClass("betamode");
        vcheck()
    })
}
...
function vcheck() {
    if ("undefined" != typeof console) {
        console.log("Asset Version Report");
        console.log("--------------------------");
        jQuery('head link[href*="css"]').each(function () {
            var a = jQuery(this).attr("href").replace(/.*\/([^\/]+).css$/g, "$1");
            jQuery("body").append('<div style="display:none" id="' + a + '"></div>');
            console.log(a + ".css => " + jQuery("#" + a).css("content"));
            jQuery("#" + a).remove()
        });
        jQuery("head script[src]").each(function () {
            if (document.location.href.indexOf("betamode=") < 0 || "true" == jQuery(this).attr("data-bmode")) {
                var a = jQuery(this).attr("src").replace(/.*\/([^\/]+).js$/g, "$1");
                console.log(a + ".js => " + fversion[a])
            }
        });
        console.log("--------------------------")
    }
...
/*! DEVICEANYWHERE */
document.location.href.indexOf("deviceanywhere") > -1 && jQuery(document).ready(function ($) {
    var a = document.createElement("script");
    a.type = "text/javascript";
    a.src = "http://webtest.deviceanywhere.com/da/js/client.js";
    $("body").append(a)
});    
