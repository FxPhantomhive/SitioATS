...
var OraclePerformance = function(a) {
        ...
        var b = {},
            ...
            h = function(b, c, d) {
                ...
                if (/\.js$/.test(c))
                    if (/jquery/.test(c)) b.jquery = d.duration;
                    else if (/(-handlebars|-chat)\.js$/.test(c));
                else if (/-base\.js$/.test(c)) b["base-js"] = d.duration;
                else {
                    b["lib-js"] = d.duration;
...
function getUrlVars() {
    var a = [],
        b, c = window.location.href; - 1 != c.indexOf("#") && (c = c.split("#")[0]);
...
var PDITLocaleMap = function($, a) {
    ...
    b.indexOf("/www-sites-stage") > -1 || b.indexOf("/www-stage") > -1 ? c = "https://www-stage.oracle.com/pdit-locale-map.json" : (b.indexOf("localhost") > -1 || b.indexOf("/webstandards") > -1) && (c = "/pdit-locale-map.json");
    ...
    function l(e, n) {
        ...
        if (n) {
            for (var t, r = 0; r < e.length; r++)
                if (e[r].url == n.url) {
                    if (n.reload && (e[r].reload = !0), n.type && !e[r].type && (e[r].type = n.type), n.name.length)
...
function bc_loadplayer(a) {
    ...
    var b = !("background" != a.ptype || !a.closest(".cw55")[0]),
        c = !!a.hasClass("bcthumbnail"),
        d = !!a.hasClass("bcgallery"),
        e = void 0 !== a.attr("data-playlistid"),
        f = "true" == a.attr("data-videotimes"),
        g = a.ptype.indexOf("playlist") > -1,
        h = void 0 === a.attr("data-options") ? "" : a.attr("data-options");
    if ("live-iframe" == a.ptype || b || c || d || e)
        if (b) {
        ...
        jQuery.when(vd01loadplayslist(i, n)).done(function(b) {
            var c, d, e, f, g, j, k, m, n, o, p, q = vd01GetBreakpoint(),
                ...
                t = 4,
                u = void 0 !== i.attr("data-pagesize") && +i.attr("data-pagesize") > 0 ? +i.attr("data-pagesize") : 12,
                v;
            i.append($(l.framework));
            if (h.indexOf("bccol3") >= 0) {
                i.find(".col4").removeClass("col4").addClass("col3");
                t = 3
            }
            v = vd01itemsPer(u, t);
            videoList = b.videos;
            i.find(".col-framework").inView();
            for (var w = 0; w < videoList.length; w++) {
                ...
                if ("duration" in d) {
                    ...
                    j = Math.floor(d.duration / 1e3 % 60);
                    j < 10 && (j = "0" + j.toString());
                    if (g > 59) {
                        f = Math.floor(g / 60);
                        g %= 60
                    ...
                    g < 10 && (g = "0" + g.toString());
                    e += f ? " (" + f + ":" + g.toString() + ":" + j.toString() + ")" : " (" + +g + ":" + j.toString() + ")"
                }
                c = i.attr("data-shareonly") ? "&shareonly=" + a.attr("data-shareonly") : "", playlistElWidth = i.outerWidth();
                k = d.thumbnail && playlistElWidth < 300 ? d.thumbnail : d.poster;
                m = '<div class="col-item' + p + '"><div class="col-item-w1"><a href="?bcid=' + d.id + c + '" class="bclink" title="' + e.replace(/\d?\d-\d?\d-\d\d\d\d/g, "").replace(/--/g, "&ndash;") + '"><button class="vjs-big-play-button"></button></a><img class="bcimg" src="' + k + '" /></div><div class="col-item-w2"><div class="bcfgallery-title"><h3>' + e.replace(/\d?\d-\d?\d-\d\d\d\d/g, "").replace(/--/g, "&ndash;") + "</h3>";
                h.indexOf("bcdesc") >= 0 && (m += n);
                h.indexOf("bcspkr") >= 0 && (m += vd01appendSpeakers(d));
                m += vd01appendFulllengthKeynote(d, i) + "</div></div></div>";
                i.find(".col-w1").append(m)
            }
            vd01speakerVis(i);
            if (r && s) {
                vd01injectBtn(i, r);
        ...
        var o = a.attr("data-bcid"),
            p = a.attr("data-shareonly") ? "&shareonly=" + a.attr("data-shareonly") : "",
            q = a.outerWidth(),
            r = a.closest(".clickvideo-overlay")[0] ? "&w10overlay=true" : "";
        jQuery.ajax({
            headers: {
                Accept: "application/json;pk=" + bc_config.pk
            },
    ...
    } else if (d) a.html('<div data-experience="' + a.attr("data-bcid") + '"></div><script src="https://players.brightcove.net/' + bc_config.account + "/experience_" + a.attr("data-bcid") + '/live.js"><\/script>');
    else {
        a.attr("class", "");
        ...
        var s = jQuery(bc_config[a.ptype].video).attr("data-player", bc_config[a.ptype].playerid).attr("data-account", bc_config.account).attr("id", a.vid);
        a.attr("data-bcid").indexOf(",") < 0 && s.attr(bc_config[a.ptype].bcidtype, a.attr("data-bcid"));
        g && jQuery("ol.vjs-playlist").addClass("vjs-playlist-tmp").removeClass("vjs-playlist");
        a.append(s);
                ...
                b.on("error", function(a) {
                    ...
                    if (isNaN(e)) {
                        ...
                        if (b.error().message) j = b.error().message;
                            ...
                            if ("VIDEO_CLOUD_ERR_NOT_PLAYABLE" == d) j = bc_errors[d].description[pglang];
                            else {
                                for (var k in f)
                                    if (d == f[k].type) {
                                        j = f[k].headline;
...
function getCookieData(a) {
    for (var b = a.length, c = document.cookie.length, d = 0, e; d < c;) {
        ...
        if (document.cookie.substring(d, f) == a) {
            e = document.cookie.indexOf(";", f); - 1 == e && (e = document.cookie.length);
            f++;
...
function sso_sign_out() {
    var a = escape(window.location.href.replace(/^http:/gi, "https:")); - 1 != a.indexOf("/secure") && (a = window.location.href.indexOf("/opn/") > -1 ? "http://www.oracle.com/opn/" : "http://www.oracle.com/partners/");
    invalidateAuthCookie();
    window.location.host.indexOf("-stage") > -1 ? window.location = "https://login-stage.oracle.com/sso/logout?p_done_url=" + a : window.location = "https://login.oracle.com/sso/logout?p_done_url=" + a
}
...
$(document).ready(function() {
    if ($(".u28")[0]) {
        ...
        if (b.indexOf("www-stage") > -1 || b.indexOf("www-portal-stage") > -1 || b.indexOf("www.stage") > -1 || b.indexOf("www-sites") > -1) var a = !0;
        var c;
        ...
        function u28hasScrolled() {
            if (!d.hasClass("dropdownactive") || !d.hasClass("u28navactive")) {
                ...
                } else Math.abs(a) + $(window).height() < $(document).height() && !d.hasClass("u28-down") && a > 0 && d.removeClass("u28-top u28-up").addClass("u28-down");
                w = a
            }
        }
        ...
        for (var A = [], B = 0; B < 10; B++) A.push("<li></li>");
        $(".u28skel").html(A.join(""));
        ...
        function buildResults(b) {
            ...
            if (a)
                if (o) var p = "https://www-portal-stage.oracle.com/search/results-nodim?No=" + b + "&Ntt=" + e + "&Dy=" + i + "&Nty=" + f + "&Ntk=" + h + "&format=json";
                else if (k && l)
                if ("us" === k && "en" === l) var p = "https://www-portal-stage.oracle.com/search/results-nodim?No=" + b + "&Ntt=" + e + "&Dy=" + i + "&Nty=" + f + "&Ntk=" + h + "&format=json";
                else var p = "https://www-portal-stage.oracle.com/search/results-nodim?No=" + b + "&Ntt=" + e + "&Dy=" + i + "&Nty=" + f + "&Ntk=" + h + "&cty=" + k + "&lang=" + l + "&format=json";
            else var p = "https://www-portal-stage.oracle.com/search/results-nodim?No=" + b + "&Ntt=" + e + "&Dy=" + i + "&Nty=" + f + "&Ntk=" + h + "&format=json";
            else if (o) var p = "/search/results-nodim?No=" + b + "&Ntt=" + e + "&Dy=" + i + "&Nty=" + f + "&Ntk=" + h + "&format=json";
            else if (k && l)
                if ("us" === k && "en" === l) var p = "/search/results-nodim?No=" + b + "&Ntt=" + e + "&Dy=" + i + "&Nty=" + f + "&Ntk=" + h + "&format=json";
                else var p = "/search/results-nodim?No=" + b + "&Ntt=" + e + "&Dy=" + i + "&Nty=" + f + "&Ntk=" + h + "&cty=" + k + "&lang=" + l + "&format=json";
            else var p = "/search/results-nodim?No=" + b + "&Ntt=" + e + "&Dy=" + i + "&Nty=" + f + "&Ntk=" + h + "&format=json";
            var q = 0,
                t = "",
                u = !1,
                v = !1,
            ...
            jQuery.getJSON(p, function(a) {
                ...
                if (f.length > 0) {
                    for (var r = 0; r < f.length; r++) {
                        ...
                        if (void 0 !== g && b <= 0 && !w) {
                            w = !0;
                            var U = g.replace(/[â€˜â€™]/g, "'").replace(/[â€œâ€]/g, '"');
                            k += U
                        }
                        if (void 0 != y && void 0 != A && !t) {
                            if (null == f[r].attributes.SourceTag) var V = !1;
                            else {
                                var V = !0;
        ...
        /*   function adjustDropdown() {
            ...
            var h = $(".u28prof"),
                ...
                l = document.getElementById("u28");
            k.observe(l, {
                attributes: !0
            });
            i <= m.outerWidth() - 30 && m.addClass("right");
            i >= m.outerWidth() && m.removeClass("right");
            if ($("body").is(".f11")) {
                j.css("width", d).css({
                    left: c.left + "px"
                });
            ...
            onload: function() {
                ...
                inputField.keydown(function(a) {
                    ...
                    if (40 === b || 9 === b && 0 == a.shiftKey) {
                        ...
                        } else n = c.last().attr("aria-selected", "true");
                        if (n) {
                            var d = n.find("a").text();
                            g.val(d)
                        }
                ...
                inputField.keyup(u28delay(function(b) {
                    var c = this.value,
                        ...
                        h = $(".u28suggest li"),
                        i = "";
                    if (a) var k = "https://www-portal-stage.oracle.com/search/askoraclesuggest/json?Ntt=" + this.value,
                        l = "https://www-portal-stage.oracle.com/search/autosuggest.json/browse?Dy=1&contentPaths=%2Fcontent%2FWeb%2FShared%2FAuto-Suggest%20Panel&templateTypes=AutoSuggestPanel&Ntt=" + this.value + "*";
                    else var k = "/search/askoraclesuggest/json?Ntx=all&Ntt=" + this.value,
                            ...
                            if (a[0].contents[0].numResults > 0) {
                                jQuery.each(a[0].contents[0].records, function(a, b) {
                                    if (a <= 2) {
                                        ...
                                        if (f) {
                                            if ("Video" === f[0]) var g = "icn-video";
                                            if ("Coversation" === f[0]) var g = "icn-chat"
                                        }
...
function buildCountryMenu(a) {
    ...
    if ($(".u10").hasClass("u10v6")) {
        var s = $(".u10").find(oracleDataMenu.classSelector(b)),
            t = s.length && s.attr("href"),
            u = $(".u10").find("#u10cmenu"),
            v = oracleDataMenu.createElement("div", ["u10menupop", k]),
            w, x;
        w = t && t.split("#")[1] || c;
        x = $("#" + w, a);
...
jQuery(document).ready(function($) {
    ...
    function activeCountrySelect() {
        ...
        if ("" !== b && null !== b) return exitACS("cookie-exist");
        var d, e, f = !!$('meta[name="siteid"]').attr("content") && $('meta[name="siteid"]').attr("content"),
            g = !!$('meta[name="countryid"]').attr("content") && $('meta[name="countryid"]').attr("content").toLowerCase(),
            h = !!$('meta[name="country"]').attr("content") && $('meta[name="country"]').attr("content"),
            i = !!$('meta[name="Language"]').attr("content") && $('meta[name="Language"]').attr("content"),
            j = $('link[hreflang="en-US"]')[0] ? ",us" : "",
            k = $('meta[name="altpages"]').attr("content") ? "," + $('meta[name="altpages"]').attr("content") + j + "," : "";
        if (!(f && g && h && i)) return exitACS("no-metadata");
        ...
        if (a.indexOf("activecountryid=") > 0) d = a.split("activecountryid=")[1].split("&")[0];
        else {
            ...
            var o = a.indexOf("www-sites") > -1 || a.indexOf("developer.oracle.com") > -1 ? "https://www.oracle.com/visitorinfo/" : "/visitorinfo/",
                p = getViCountryId(o);
            if (p.country_code && 0 == c && l.indexOf("," + p.country_code.toLowerCase() + ",") > -1) return exitACS("truste-implied-failed");
            if (!p.country_code) {
        ...
        var q = !1,
            r = [],
            s, t = "https://www.oracle.com/pdit-locale-map.json";
        a.indexOf("/www-sites-stage") > -1 || a.indexOf("/www-stage") > -1 ? t = "https://www-stage.oracle.com/pdit-locale-map.json" : (a.indexOf("localhost") > -1 || a.indexOf("/webstandards") > -1) && (t = "/pdit-locale-map.json");
        ...
        var u = !1,
            ...
            y = "",
            z = "",
            A = a.indexOf("betamode=webstandards") > -1 ? "https://webstandards.us.oracle.com:89https://www.oracle.com/asset/web/json/ac-translation-data.json" : "https://www.oracle.com/asset/web/json/ac-translation-data.json";
        ...
        translationCheck();
        if (u) return exitACS(u, !0);
        if ("" == v) return exitACS("no-translation-found");
        var B = "";
        $("body").prepend($("<div/>").addClass("u24 u24v0 darktheme").attr("data-trackas", "active-country-select"));
        if ($(".f20")[0]) $(".u24").prepend($("<div/>").addClass("u24w1 rw-globe"));
        else {
            B = "icn-none";
        ...
        $.each(s, function(a, b) {
            if (r[b.siteid][0].toLowerCase() != d || "default" == b.siteid);
                ...
                "fr" === e && $(".u24w3 ul").prepend('<li><a href="/' + b.siteid + n + '">' + b.countryname + " - FranÃ§ais</a></li>");
...
$(document).on("click", "a.sharelink", function(a) {
    ...
    if ($(this).is("[data-esubject]"))
        if ($(this).attr("data-esubject").indexOf("%PAGETITLE%") > -1) var c = document.title,
            d = $(this).attr("data-esubject").replace(/%PAGETITLE%/g, c);
        else var d = $(this).attr("data-esubject");
    ...
    if ("facebook" == $(this).attr("data-sharetype")) window.open("https://www.facebook.com/sharer/sharer.php?u=" + b, "fb-share", "height=300,width=400");
...
jQuery(document).ready(function($) {
    ...
    function getLightboxType(a) {
        var b = a.attr("href"),
            c = new RegExp(i.origin + i.pathname, "i"),
            d = "zoom-gallery" === a.data("lbl"),
            f, j, k, l;
        b = b ? b.replace(c, "") : "";
        if (/^https?:\/\//i.test(b)) {
            l = b.split("//")[1].split("/")[0];
    ...
    function onExternalContentLoaded(a) {
        var b = $("> .w11 .w11w6 iframe", g),
            c = $("> .w11 .w11w4", g),
            e = $("> .w11 .w11w5", g),
            f, h, i, j, k, l, m, n;
        if (!d) return lightboxFadeIn();
        try {
